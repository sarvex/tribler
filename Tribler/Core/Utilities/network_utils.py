...
def get_random_port(socket_type="all", min_port=5000, max_port=60000):
    ...
    assert socket_type in (
        ...
        "tcp",
        "udp",
    ), f"Invalid socket type {type(socket_type)}"
    ...
    assert (
        0 < min_port <= max_port <= 65535
    ), f"Invalid min_port and mac_port values {min_port}, {max_port}"
...
def _test_port(family, sock_type, port):
    ...
    assert sock_type in (
        socket.SOCK_DGRAM,
        socket.SOCK_STREAM,
    ), f"Invalid sock_type value {sock_type}"
    assert 0 < port <= 65535, f"Invalid port value {port}"
